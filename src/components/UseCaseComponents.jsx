...
const UseCaseComponents = () => {
  ...
  return (
    <section className="flex w-full flex-col justify-around gap-10 md:flex-row ">
      <Card className="h-96 w-full bg-background/100 p-5 md:w-2/5 dark:bg-default-100/30">
    ...
    <div>
      <Listbox>
        {promptUseCases.map((caseItem, i) => {
          return (
            <ListboxSection aria-label="a" showDivider>
              <ListboxItem textValue={caseItem.value}>
                ...
                >
                  <div
                    className={`${bgColor(caseItem.color)} rounded-full p-2`}
                  >
                    {caseItem.icon}
                  </div>
...
const CardComponent = ({ activeBtn }) => {
  return (
    <Card className="flex flex-col justify-between bg-background/100  p-5 sm:w-full md:w-2/5 dark:bg-default-100/30">
      <section>
        <div
          className={`${bgColor(
            promptUseCases[activeBtn].color
          )} flex h-20 w-20 items-center justify-center rounded-full p-2 transition`}
        >
